...
const int EMPTY_SIZE = 8;

const int HW_TYPE_OFFSET = 0;
const int PROTOCOL_TYPE_OFFSET = 2;
const int HW_SIZE_OFFSET = 4;
...
TEST(ArpFactory, defaultHardwareType)
{
    networkpackets::ArpFactory factory(0x1234, 0, 0);
    std::shared_ptr<networkutils::Packet> packet = factory.getPacket();
    const char* type = packet->data() + HW_TYPE_OFFSET;
...
TEST(ArpFactory, defaultProtocolType)
{
    networkpackets::ArpFactory factory(0x1234, 0, 0);
    std::shared_ptr<networkutils::Packet> packet = factory.getPacket();
    const char* type = packet->data() + PROTOCOL_TYPE_OFFSET;
...
TEST(ArpFactory, defaultHardwareSize)
{
    networkpackets::ArpFactory factory(0x1234, 0, 0);
    std::shared_ptr<networkutils::Packet> packet = factory.getPacket();
    const char* size = packet->data() + HW_SIZE_OFFSET;
...
TEST(ArpFactory, defaultProtocolSize)
{
    networkpackets::ArpFactory factory(0x1234, 0, 0);
    std::shared_ptr<networkutils::Packet> packet = factory.getPacket();
    const char* size = packet->data() + PROTOCOL_SIZE_OFFSET;
...
TEST(ArpFactory, opCode)
{
    networkpackets::ArpFactory factory(0x1234, 0, 0);
    std::shared_ptr<networkutils::Packet> packet = factory.getPacket();
    const char* type = packet->data() + OPCODE_OFFSET;
...
TEST(ArpFactory, payload)
{
    ...
    const char* payload = packet->data() + EMPTY_SIZE;
